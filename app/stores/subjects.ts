...
export const useSubjectsStore = defineStore('subject', () => {
  ...
  function previousSubject() {
    if (activeSubjectIndex.value === null || activeSubjectIndex.value < 0) return
    const subject = subjects.value[activeSubjectIndex.value]
    if (subject) {
  ...
  function isMovable(id: number): { up: boolean; down: boolean } {
    ...
    const isMovableUp = index > 0 && subjects.value[index - 1]?.status === 'upcoming'
    const isMovableDown =
      index < subjects.value.length - 1 && subjects.value[index]?.status === 'upcoming'
