  ...
  <div class="p-2">
    <ol>
      <li
        ...
        class="mb-2 flex justify-between border-l-4 p-2 items-center relative transition-all duration-300"
      >
        <!-- bookmark img -->
        ...
        <img
          ...
          alt="bookmark"
          class="size-8 rounded-none absolute -left-6 top-0"
        />

        <div class="flex items-start gap-1 flex-col flex-1 min-w-0 ms-1.5">
          <p class="text-xl overflow-hidden wrap-break-word w-full pr-12">
            {{ subject.title }}
          </p>
          <ul
            v-if="!props.small && subject.sources.length > 0"
            class="whitespace-nowrap overflow-hidden text-ellipsis w-full pr-16"
          >
            <li
        ...
        <div v-if="!props.small" class="flex justify-between gap-4">
          <div class="flex items-end justify-center absolute bottom-0 right-0">
            ...
            <UBadge
              v-if="subject.startTime"
              size="xs"
              class="uppercase text-white text-lg font-normal bg-slate-950/40"
            >
              {{
            ...
            <UBadge
              v-else
              size="xs"
              class="uppercase font-normal text-white text-sm bg-gray-900/80"
            >
              À venir
            </UBadge>
          </div>
